...
| AI 识图 | 图片文字乱码（mojibake）时，把页面渲染成 PNG，用多模态视觉识读 |
...
### Step 2.5 — AI 识图救援（图片文字乱码时）

**触发条件（任一）**：detect_math_code 返回的 math 多为乱码（`$...$` 标签、无法映射的字形）；总结/提问依赖某张图；图注能看出内容但正文文字层不可读；页面文字极少但有图。

流程：
1. 用 `search_pdf.py "<json>" "图" --top 50` 定位候选图页（0-based 页码）
2. 渲染页面为 PNG：`export_images.py "<PDF>" --pages 59,60,67 --out "<工作区>/_pdf_images" --dpi 150`（装不了 PyMuPDF 时用 `--embedded` 回退提取嵌入图）
...
| 图片乱码 | Step 2.5 渲染 PNG + 视觉识读，引用"（图2.15，PDF p.59，据视觉解读）"，不猜 |
